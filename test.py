...
def plotData(data,columnColour,maxDate,minDate): 
    ...
    for i in range(0,len(data)):
        ...
        # Else, add to given day
            ...
            currentDate -= dt.timedelta(days=0.5)
            tomorrow = currentDate + dt.timedelta(days=1)
            plt.axvspan(xmin=currentDate, xmax=tomorrow, ymin=currentDataItem.startTime, ymax=currentDataItem.stopTime, facecolor=currentDataItem.activityIndex, alpha=0.5)
...
def formatDataForPlot(listOfInputLists):
    ...
    for i in range(1,len(listOfInputLists)):
        activities.append(activityInstance(listOfInputLists[i]))
    return activities
